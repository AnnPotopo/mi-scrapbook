...
const CANVAS_BACKGROUNDS = {
    ...
    notebook: { name: 'Escolar', className: 'bg-[#ffffff] bg-[linear-gradient(transparent_95%,#cbd5e1_95%)] bg-[length:100%_1.5rem] border-l-[40px] border-rose-100' },
    kraft: { name: 'Papel Kraft', className: 'bg-[#d4b595] bg-[url("https://www.transparenttextures.com/patterns/cardboard-flat.png")]' },
    wood: { name: 'Madera', className: 'bg-[#8b5a2b] bg-[url("https://www.transparenttextures.com/patterns/wood-pattern.png")]' },
    cork: { name: 'Corcho', className: 'bg-[#d2a679] bg-[url("https://www.transparenttextures.com/patterns/cork-board.png")]' },
    marble: { name: 'Mármol', className: 'bg-[#f0f0f0] bg-[url("https://www.transparenttextures.com/patterns/white-marble.png")]' },
    ...
    dark: { name: 'Oscuro', className: 'bg-stone-900' },
    starry: { name: 'Estrellas', className: 'bg-[#0a0f1d] bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:40px_40px] shadow-[inset_0_0_100px_rgba(0,0,0,0.8)]' },
    ocean: { name: 'Océano', className: 'bg-[#006994] bg-[radial-gradient(circle_at_top,_#00b4db_0%,_#0083b0_50%,_#000046_100%)]' },
    galaxy: { name: 'Galaxia', className: 'bg-[#090a0f] bg-[radial-gradient(ellipse_at_center,_#3b0066_0%,_#12002b_50%,_#000000_100%)]' },
    forest: { name: 'Bosque', className: 'bg-[#1e3323] bg-[radial-gradient(circle_at_bottom,_#2d5a36_0%,_#1e3323_60%,_#0a140d_100%)]' },
    mountains: { name: 'Montañas', className: 'bg-[#1d232a] bg-[linear-gradient(to_bottom,#2c3e50_0%,#1d232a_100%)]' }
};
...
function InteractableGroup({ folder, items, updateMultiplePhotos, isEditMode, isSelected, onSelect, zoom, isLocked }) {
    ...
    const maxZ = Math.round(Math.max(...items.map(i => i.zIndex || 0), 1));
...
export default function ScrapbookCanvas({ user, activeAlbum, activePhotos, setCurrentView, setActiveAlbumId, setDbError }) {
    ...
    // VARIABLES DE ARRASTRE PARA LAS CAPAS
    ...
    // --- CÁLCULO Y ORDENAMIENTO DE CAPAS ---
    const folders = activePhotos.filter(p => p.type === 'folder').sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0));
    ...
    const menuRootEntities = [...folders, ...items.filter(i => !i.folderId)].sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0));

    const canvasFreeItems = items.filter(i => {
    ...
    const multiSelectedItems = canvasFreeItems.filter(i => selectedItemIds.includes(i.id));
    const renderableCanvasFreeItems = canvasFreeItems.filter(i => {
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i].type.startsWith('image/')) {
                    ...
                    await setDoc(doc(collection(db, 'artifacts', appId, 'photos')), {
                        ...
                        width: 300, rotation: Math.floor(Math.random() * 20) - 10, zIndex: Math.round(maxZ) + i + 1,
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i].type.startsWith('image/')) {
                    ...
                    await setDoc(doc(collection(db, 'artifacts', appId, 'photos')), {
                        ...
                        width: 150, rotation: 0, zIndex: Math.round(maxZ) + i + 1,
    ...
    const handleCreateFolder = async () => {
        try {
            ...
            await setDoc(newFolderRef, {
                albumId: activeAlbum.id, type: 'folder', content: 'Nueva Carpeta', color: '#3b82f6', createdAt: Date.now(), isGrouped: false, isHidden: false, isLocked: false, zIndex: Math.round(maxZ) + 10
            });
        } catch (error) { if (error.code === 'permission-denied') setDbError('permissions'); }
    };
    ...
    const finishDrawingLazo = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                zIndex: Math.round(maxZ) + 1, color: drawingLazoType === 'lazo_guia' ? '#3b82f6' : '#1f2937',
    ...
    const handleSaveSticker = async (type) => {
        ...
        if (type === 'drawing') {
            ...
            try {
                if (editingStickerId) await updatePhoto(editingStickerId, drawingData);
                    ...
                    await setDoc(newStickerRef, { albumId: activeAlbum.id, ...drawingData, x: startX, y: startY, width: 400, zIndex: Math.round(maxZ) + 1, isLocked: false, folderId: null });
        ...
        try {
            if (editingStickerId) await updatePhoto(editingStickerId, stickerData);
                ...
                const newStickerRef = doc(collection(db, 'artifacts', appId, 'photos'));
                await setDoc(newStickerRef, { albumId: activeAlbum.id, ...stickerData, x: startX, y: startY, width, zIndex: Math.round(maxZ) + 1, isLocked: false, folderId: null });
                setSelectedItemIds([newStickerRef.id]);
            }
            setStickerModalType(null); setEditingStickerId(null); setShowStickerMenu(false);
        } catch (error) { if (error.code === 'permission-denied') setDbError('permissions'); }
    ...
    const handleAddEmoji = async (emoji, isAnim = false) => {
        ...
        try {
            ...
            await setDoc(newEmojiRef, {
                ...
                zIndex: Math.round(maxZ) + 1, animationStyle: isAnim ? 'animate-bounce' : null, isLocked: false, folderId: null
            });
    ...
    const handleMoveLayer = async (item, direction) => {
        const allEntities = [...folders, ...items].sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0));
        const siblings = item.folderId
            ? items.filter(e => e.folderId === item.folderId).sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0))
            : allEntities.filter(e => !e.folderId);

        const currentIndex = siblings.findIndex(s => s.id === item.id);

        if (direction === 'up' && currentIndex > 0) {
            const upperItem = siblings[currentIndex - 1];
            let newZ = Math.round(upperItem.zIndex || 0);
            let itemZ = Math.round(item.zIndex || 0);
            if (newZ === itemZ) newZ += 1; // Forzamos desempatar
            await Promise.all([
                updatePhoto(item.id, { zIndex: newZ }),
                updatePhoto(upperItem.id, { zIndex: itemZ })
            ]);
        } else if (direction === 'down' && currentIndex < siblings.length - 1) {
            const lowerItem = siblings[currentIndex + 1];
            let newZ = Math.round(lowerItem.zIndex || 0);
            let itemZ = Math.round(item.zIndex || 0);
            if (newZ === itemZ) itemZ += 1;
            await Promise.all([
                updatePhoto(item.id, { zIndex: newZ }),
                updatePhoto(lowerItem.id, { zIndex: itemZ })
            ]);
        } else if (direction === 'top') {
            const maxZ = Math.max(...allEntities.map(i => i.zIndex || 0), 0);
            await updatePhoto(item.id, { zIndex: Math.round(maxZ) + 1 });
        }
    };

    const handleDropOnEntity = async (draggedId, targetEntity) => {
        const dragged = [...folders, ...items].find(e => e.id === draggedId);
        if (!dragged || dragged.id === targetEntity.id) return;

        if (targetEntity.type === 'folder' && dragged.type !== 'folder') {
            const childrenInsideFolder = items.filter(i => i.folderId === targetEntity.id);
            const maxChildZ = childrenInsideFolder.length > 0 ? Math.max(...childrenInsideFolder.map(c => c.zIndex || 0)) : (targetEntity.zIndex || 0);
            await updatePhoto(dragged.id, { folderId: targetEntity.id, zIndex: Math.round(maxChildZ) + 1 });
            setOpenFolders(prev => ({ ...prev, [targetEntity.id]: true }));
        } else {
            await updatePhoto(dragged.id, { folderId: targetEntity.folderId || null, zIndex: Math.round(targetEntity.zIndex || 0) + 1 });
        }
    };
    ...
    const renderLayerItem = (p) => {
        ...
        let isGroupedMember = false;
        let isVisuallyHidden = p.isHidden;
        let isLocked = p.isLocked;

        if (!isFolder && p.folderId) {
            const pFolder = folders.find(f => f.id === p.folderId);
            isGroupedMember = pFolder && pFolder.isGrouped;
            isVisuallyHidden = p.isHidden || pFolder?.isHidden;
            isLocked = p.isLocked || pFolder?.isLocked;
        }

        let displayName = '';
        if (isFolder) displayName = p.content;
        else if (p.type === 'emoji') displayName = p.content;
        else displayName = p.type === 'postit' ? 'Post-it' : p.type === 'date' ? 'Fecha' : p.type === 'location' ? 'Ubicación' : p.type === 'link' ? 'Enlace' : p.type === 'music' ? 'Canción' : p.type === 'counter' ? 'Contador' : p.type === 'lazo' ? 'Lazo' : p.type === 'lazo_guia' ? 'Lazo Guía' : p.type === 'drawing' ? 'Dibujo Libre' : p.type === 'animated' ? 'Animación' : p.type === 'custom_sticker' ? 'Pegatina PNG' : 'Imagen';

        return (
            <div key={p.id} className={`flex flex-col mb-1 ${isVisuallyHidden ? 'opacity-40' : ''}`}>
                <div
                    draggable
                    onDragStart={(e) => { e.dataTransfer.setData('itemId', p.id); }}
                    onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); setDragOverItemId(p.id); }}
                    onDragLeave={(e) => { e.preventDefault(); e.stopPropagation(); setDragOverItemId(null); }}
                    onDrop={(e) => {
                        e.preventDefault(); e.stopPropagation();
                        setDragOverItemId(null);
                        const draggedId = e.dataTransfer.getData('itemId');
                        if (draggedId) handleDropOnEntity(draggedId, p);
                    }}
                    onClick={() => setSelectedItemIds([isGroupedMember ? p.folderId : p.id])}
                    className={`flex items-center justify-between p-2 rounded-lg text-sm group cursor-grab active:cursor-grabbing transition-all border ${selectedItemIds.includes(isGroupedMember ? p.folderId : p.id) ? 'bg-indigo-50 border-indigo-300 shadow-sm scale-[1.02]' : (isFolder ? (p.isGrouped ? 'bg-indigo-50/50' : 'bg-stone-50 hover:bg-stone-100') : 'bg-white border-transparent hover:border-stone-200')} ${isLocked ? 'grayscale' : ''} ${dragOverItemId === p.id ? (isFolder ? 'ring-2 ring-blue-500' : 'border-t-2 border-t-blue-500') : ''}`}
                >
                    <div className="flex items-center gap-1.5 pointer-events-auto relative w-24">
                        {isFolder && (
                            <button onClick={(e) => { e.stopPropagation(); setOpenFolders(prev => ({ ...prev, [p.id]: !prev[p.id] })) }} className={`p-0.5 hover:bg-stone-200 rounded transform transition-transform ${openFolders[p.id] ? 'rotate-90' : ''}`}>
                                <ChevronRight size={14} className={p.isGrouped ? 'text-indigo-500' : 'text-stone-500'} />
                            </button>
                        )}
                        {!isFolder && <div className="w-5" />}

                        {isFolder && <Folder size={14} fill={p.color} color={p.color} className="shrink-0" />}
                        <span className={`truncate font-medium pointer-events-none ${isFolder ? 'text-xs' : 'text-[11px]'} ${selectedItemIds.includes(isGroupedMember ? p.folderId : p.id) ? 'text-indigo-800' : 'text-stone-600'}`}>
                            {displayName}
                        </span>
                    </div>

                    <div className="flex items-center gap-0.5 opacity-40 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); updatePhoto(p.id, { isHidden: !p.isHidden }); }} className={`p-1 hover:bg-stone-200 rounded transition-colors ${p.isHidden ? 'text-stone-300' : 'text-stone-600'}`} title={p.isHidden ? 'Mostrar' : 'Ocultar'}>
                        ...
                        {isFolder && (
                            <button onClick={(e) => { e.stopPropagation(); updatePhoto(p.id, { isGrouped: !p.isGrouped }); }} className={`p-1 hover:bg-indigo-200 rounded transition-colors ${p.isGrouped ? 'text-indigo-600 bg-indigo-200' : 'text-stone-400'}`} title={p.isGrouped ? 'Desagrupar' : 'Agrupar'}><Layers size={14} /></button>
                        )}

                        <div className="flex bg-stone-100 rounded mx-0.5">
                            <button onClick={(e) => { e.stopPropagation(); handleMoveLayer(p, 'down'); }} className="p-1 hover:bg-stone-200 text-stone-600 transition-all rounded-l" title="Bajar"><ArrowDown size={12} /></button>
                            <button onClick={(e) => { e.stopPropagation(); handleMoveLayer(p, 'up'); }} className="p-1 hover:bg-stone-200 text-stone-600 transition-all rounded-r" title="Subir"><ArrowUp size={12} /></button>
                        </div>

                        {!isFolder && <button onClick={(e) => { e.stopPropagation(); handleMoveLayer(p, 'top'); }} className="p-1 hover:bg-indigo-100 text-indigo-600 rounded transition-all" title="Hasta el frente"><ChevronsUp size={12} /></button>}

                        {(isEditable || isFolder) && <button onClick={(e) => { e.stopPropagation(); openStickerModal(p.type, p); }} className="p-1 hover:bg-blue-100 text-blue-600 rounded transition-all" title="Editar"><Edit2 size={12} /></button>}
                ...
                {isFolder && openFolders[p.id] && (
                    <div className={`p-1.5 ml-2 mt-1 flex flex-col border-l-2 ${p.isGrouped ? 'bg-indigo-50/30 border-indigo-200' : 'bg-stone-50/50 border-stone-200'}`}>
                        {items.filter(i => i.folderId === p.id).sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0)).map(child => renderLayerItem(child))}
                        {items.filter(i => i.folderId === p.id).length === 0 && <p className="text-[10px] text-stone-400 text-center py-2 italic pointer-events-none">Carpeta vacía</p>}
                    </div>
                )}
            </div>
        );
    ...
    return (
        <div className="h-screen flex flex-col font-sans overflow-hidden bg-stone-100 relative">
                    ...
                    {isEditMode && (
                        <>
                            <div className="relative">
                                ...
                                {showBgMenu && (
                                    <div className="absolute top-full right-0 mt-3 bg-white rounded-2xl shadow-xl border border-stone-100 p-4 w-72 z-[99999] max-h-96 overflow-y-auto">
            ...
            {/* MENÚ DE CAPAS UNIFICADO (CARPETAS E ÍTEMS) */}
            {showLayers && isEditMode && (
                <div className="absolute top-24 right-6 w-80 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl p-4 z-[99999] border border-stone-200 print:hidden flex flex-col max-h-[80vh]">
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 pb-2">
                        {menuRootEntities.map(entity => renderLayerItem(entity))}

                        <div
                            className={`mt-4 pt-2 flex flex-col gap-1 pb-4 min-h-[60px] rounded-xl transition-all border-2 ${dragOverFolderId === 'root' ? 'border-dashed border-stone-400 bg-stone-100/50' : 'border-transparent'}`}
                            ...
                            onDrop={(e) => {
                                ...
                                if (itemId) {
                                    ...
                                    updatePhoto(itemId, { folderId: null, zIndex: Math.round(maxZ) + 1 });
                                }
                            }}
                        >
                            <p className="text-[10px] text-stone-400 text-center py-4 italic pointer-events-none">Arrastra objetos aquí para sacarlos de sus carpetas</p>
                        </div>
                    </div>
                </div>
            )}

            {/* CAJA DE SELECCIÓN MÚLTIPLE VISUAL */}
            {selectionBox && selectionBox.active && (
                <div
                    className="absolute border-2 border-blue-400 bg-blue-400/20 pointer-events-none z-[9999] rounded-sm"
                    style={{
                        ...
                        transform: scrollContainerRef.current ? `translate(${-scrollContainerRef.current.scrollLeft}px, ${-scrollContainerRef.current.scrollTop}px)` : 'none'
                    }}
                />
            )}
            ...
            >
                ...
                >
                    ...
                    <div className="absolute inset-0 w-full h-full pointer-events-none z-10">
                        ...
                        {renderableCanvasFreeItems.map(photo => {
                            ...
                            return (
                                <InteractablePhoto
                                    ...
                                    onClickView={(p) => setSelectedPhotoForDesc(p)} onEditClick={(p) => openStickerModal(p.type, p)}
